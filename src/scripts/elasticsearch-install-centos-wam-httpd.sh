...
node_is_up()
{
# need to confirm that --fail was in place to detect if failed to find master
  exec 17>&1
...
apply_security_settings()
{
    ...
    local XPACK_SECURITY_PATH="_security"
...
configure_awareness_attributes()
{
  ...
  if [[ -n $METADATA ]]; then
    ...
    log "[configure_awareness_attributes] WARNING could not fetch VM-instance metadata"
  fi
}
...
configure_os_properties()
{
    ...
    # DNS Retry
    echo 'RES_OPTIONS="timeout:10 attempts:5"' >> /etc/sysconfig/network
    #resolvconf -u
...
install_ntp()
{
    install_yum_package ntp
    install_yum_package ntpdate
...
{
    #configure self signed ssl cert on httpd for reverse proxy to client node
    if [ ${CLIENT_ONLY_NODE} -ne 0 ]; then
        log "[client_node_httpd_self_signed_cert] Configure client node for httpd reverse proxy using self signed ssl cert"
        bash httpdrevproxyselfsigned.sh -P 9200
        if [ $? == 0 ]; then
            log "[client_node_httpd_self_signed_cert] Configure client node httpd for certificate authentication"
            bash clientnodecertauth.sh
        fi
